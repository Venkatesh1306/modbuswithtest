...
#define READ(x,y)  ((0u == (x & (1<<y)))?0u:1u)
 ...
 uint16_t Txtempbuf[100];
 unsigned int byte_count, h, bit_index, limit, coil_bit;
...
uint16_t
readCoilStatus(uint8_t *p_modbusTxBuf, uint16_t *p_dataMemory, mbPacketParse_t *p_parseModbusTcpData) {
    ...
    for (byte_index = 0; byte_index < (regTx + 1); byte_index++) {
        ...
        p_modbusTxBuf[9 + wordOffset] = (Txtempbuf[byte_index] % 0x100);
        p_modbusTxBuf[10 + wordOffset] = (Txtempbuf[byte_index] / 0x100);
    }
